<template>
  <q-page class="applications-page q-pa-md">
    <div class="page-shell">
      ...
      <ApplicationTable :rows="filteredRows" :loading="loading" :saving="saving" @view="openView" @delete="deleteApplication" @bind-resume="openBinding" @preview-resume="openResumePreview" @status-change="onStatusChange" />
    </div>
    <ApplicationFormDialog v-model="showCreate" :opportunities="opportunities" :resumes="resumes" :saving="saving" @save="onCreate" />
    ...
    <ApplicationResumeBindingDialog v-model="showBinding" :application="bindingApplication" :resumes="resumes" :saving="saving" @save="onBindResume" />
    <ResumePreviewDialog v-model="showResumePreview" :resume="previewResume" :preview-url="previewUrl" :preview-text="previewText" :loading="previewLoading" :error="previewError" @download="previewResume && downloadResume(previewResume)" />
  </q-page>
</template>
...
const $q = useQuasar();
const { opportunities, resumes, rows, filteredRows, tabCounts, filters, activeTab, loading, saving, load, clearFilters, updateStatus, createApplication, bindResume, deleteApplication } = useApplications();
...
function openBinding(row: ApplicationRow) { bindingApplication.value = row; showBinding.value = true; }
async function onBindResume(resumeId: string | null) {
  if (!bindingApplication.value) return;
  if (await bindResume(bindingApplication.value.id, resumeId)) showBinding.value = false;
}
...
async function onCreate(payload: { opportunity_id: string; resume_id: string | null; status: ApplicationStatus; applied_date: string | null; notes: string | null }) {
...
async function openResumePreview(resume: Resume) {
  ...
  try {
    ...
    if (["TXT", "MD"].includes(fileTypeLabel(resume))) previewText.value = await blob.text();
    else {
      const previewBlob = fileTypeLabel(resume) === "PDF" && blob.type !== "application/pdf" ? new Blob([blob], { type: "application/pdf" }) : blob;
      previewUrl.value = URL.createObjectURL(previewBlob);
    }
  } catch {
    previewError.value = true;
    $q.notify({ type: "negative", message: "Could not preview attached resume" });
  } finally {
...
async function downloadResume(resume: Resume) {
  try {
    ...
    link.href = url;
    link.download = resume.file_name || `${resume.name}.${fileTypeLabel(resume).toLowerCase()}`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
  } catch {
    $q.notify({ type: "negative", message: "Could not download attached resume" });
  }
}
...
watch(showResumePreview, open => { if (!open) { clearPreviewUrl(); previewText.value = null; previewResume.value = null; previewError.value = false; } });
