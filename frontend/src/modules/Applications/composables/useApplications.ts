...
export function useApplications() {
  ...
  const rows = computed<ApplicationRow[]>(() =>
    applications.value.map(application => ({
      ...
      resume:
        resumes.value.find(item => item.id === application.resume_id) ?? null
    }))
  );
  ...
  const filteredRows = computed(() =>
    rows.value.filter(row => {
      ...
      if (
        filters.notes &&
        !(row.notes ?? "").toLowerCase().includes(filters.notes.toLowerCase())
      )
        return false;
      if (
        filters.appliedFrom &&
        (!row.applied_date || row.applied_date < filters.appliedFrom)
      )
        return false;
      if (
        filters.appliedTo &&
        (!row.applied_date || row.applied_date > filters.appliedTo)
      )
        return false;
      if (filters.hasResume === "yes" && !row.resume_id) return false;
      if (filters.hasResume === "no" && row.resume_id) return false;
      if (filters.hasCoverLetter === "yes" && !row.cover_letter_id)
        return false;
  ...
  const tabCounts = computed<Record<ApplicationTabKey, number>>(() => ({
    ...
    interviews: rows.value.filter(row => tabMatches(row.status, "interviews"))
      .length,
  ...
  async function load(): Promise<void> {
    loading.value = true;
    try {
      const [appList, opportunityList, resumeList] = await Promise.all([
        applicationApi.list(),
        opportunityApi.list(),
        resumeApi.list()
      ]);
  ...
  async function updateStatus(
    ...
    const previous = applications.value.find(
    ...
    try {
      ...
      $q.notify({
        type: "positive",
        message: `Status updated to ${APPLICATION_STATUS_LABELS[status]}`
      });
    } catch {
      $q.notify({
        type: "negative",
  ...
  async function createApplication(
    payload: Parameters<typeof applicationApi.create>[0]
  ): Promise<boolean> {
    saving.value = true;
    try {
  ...
  async function bindResume(
    ...
    try {
      ...
      $q.notify({
        type: "positive",
        message: resumeId
          ? "Resume binding updated"
          : "Resume removed from application"
      });
      return true;
    } catch {
      $q.notify({
        type: "negative",
        message: "Could not update the application resume"
      });
      return false;
    } finally {
      saving.value = false;
  ...
  async function deleteApplication(row: ApplicationRow): Promise<void> {
    const confirmed = await new Promise<boolean>(resolve => {
      $q.dialog({
        ...
        message: `Remove the application for “${row.opportunity?.title ?? "this opportunity"}”?`,
        cancel: true,
        persistent: true
      })
        .onOk(() => resolve(true))
    ...
    try {
      ...
      applications.value = applications.value.filter(
        application => application.id !== row.id
      );
