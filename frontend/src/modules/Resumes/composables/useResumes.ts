...
export function useResumes() {
  ...
  const filteredRows = computed(() =>
    resumes.value.filter(row => {
      ...
      if (filters.fileType && fileTypeLabel(row) !== filters.fileType)
        return false;
      if (
        filters.attachment === "attached" &&
        row.linkedApplications.length === 0
      )
        return false;
      if (
        filters.attachment === "unattached" &&
        row.linkedApplications.length > 0
      )
        return false;
      const createdDate = row.created_at.slice(0, 10);
      if (filters.uploadedFrom && createdDate < filters.uploadedFrom)
        return false;
  ...
  const tabCounts = computed<Record<ResumeTabKey, number>>(() => ({
    ...
    unattached: resumes.value.filter(row => tabMatches(row, "unattached"))
      .length
  }));
  ...
  async function load(): Promise<void> {
    loading.value = true;
    try {
      const [resumeList, applicationList, opportunityList] = await Promise.all([
        resumeApi.list(),
        applicationApi.list(),
        opportunityApi.list()
      ]);
      const opportunityNames = new Map(
        opportunityList.map(opportunity => [opportunity.id, opportunity.title])
      );
      ...
      for (const application of applicationList) {
        ...
        linked.push({
          ...
          opportunityTitle:
            opportunityNames.get(application.opportunity_id) ??
            "Unknown opportunity"
        });
        linkedByResume.set(application.resume_id, linked);
      }
      resumes.value = resumeList.map(resume => ({
        ...resume,
        linkedApplications: linkedByResume.get(resume.id) ?? []
      }));
    } catch {
  ...
  async function deleteResume(row: ResumeRow): Promise<void> {
    if (row.linkedApplications.length > 0) {
      $q.notify({
        type: "warning",
        message: "Attached resumes cannot be deleted"
      });
      return;
    }
