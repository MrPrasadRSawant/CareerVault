  ...
  <q-dialog :model-value="modelValue" persistent @update:model-value="$emit('update:modelValue', $event)">
    <q-card class="opportunity-dialog">
      <q-card-section class="dialog-header">
        <div>
          <div class="dialog-title">{{ editingOpportunity ? "Edit opportunity" : "New opportunity" }}</div>
      ...
      <q-card-section class="dialog-body">
        <q-form class="q-gutter-md" @submit.prevent="save">
          <q-stepper v-model="wizardStep" flat animated class="opportunity-stepper">
            <q-step :name="1" title="Role" caption="Essentials" icon="work_outline" :done="wizardStep > 1">
              ...
              <div class="row q-col-gutter-md">
                <div class="col-12 col-md-7"><q-input v-model="form.title" outlined label="Role title *" :rules="[isRequired]" /></div>
                <div class="col-12 col-md-5"><q-input v-model="form.company_name" outlined label="Company name" /></div>
                <div class="col-12 col-md-6"><q-input v-model="form.job_location" outlined label="Job location" placeholder="Remote, London, Hybrid…" /></div>
                <div class="col-12 col-md-6"><q-input v-model="form.experience_level" outlined label="Expected experience" placeholder="3–5 years" /></div>
              </div>
            </q-step>
            <q-step :name="2" title="Source" caption="Original links" icon="link" :done="wizardStep > 2">
              ...
              <div class="row q-col-gutter-md">
                <div class="col-12"><q-input v-model="form.post_url" outlined label="Post URL" type="url" /></div>
                <div class="col-12 col-md-6"><q-input v-model="form.company_career_page" outlined label="Company career page" type="url" /></div>
                <div class="col-12 col-md-6"><q-input v-model="form.company_url" outlined label="Company website" type="url" /></div>
                <div class="col-12 col-md-6"><q-input v-model="form.posted_on_utc" outlined label="Posted date/time (UTC)" type="datetime-local" hint="Optional — stored in UTC" /></div>
              </div>
            </q-step>
            <q-step :name="3" title="Evaluate" caption="Next action" icon="fact_check">
              ...
              <div class="row q-col-gutter-md">
                <div class="col-12 col-md-6"><q-select v-model="form.status" outlined emit-value map-options label="Opportunity status" :options="statusOptions" /></div>
                <div class="col-12"><q-input v-model="form.required_skills" outlined label="Skills asked for" hint="Separate skills with commas" /></div>
                <div class="col-12"><q-input v-model="form.description" outlined label="Job description" type="textarea" autogrow /></div>
              </div>
            </q-step>
          </q-stepper>
          <div class="dialog-actions">
            ...
            <q-btn v-if="wizardStep < 3" unelevated no-caps color="primary" label="Continue" icon-right="arrow_forward" @click="nextStep" />
            <q-btn v-if="wizardStep === 3" unelevated no-caps color="primary" :label="editingOpportunity ? 'Save changes' : 'Save opportunity'" type="submit" :loading="saving" />
          </div>
        </q-form>
      </q-card-section>
    </q-card>
...
function emptyForm(): OpportunityForm {
  return { title: "", company_name: "", post_url: "", company_career_page: "", company_url: "", posted_on_utc: "", job_location: "", description: "", required_skills: "", experience_level: "", status: "saved" };
}
...
watch([() => props.modelValue, () => props.editingOpportunity], ([open]) => {
  ...
  Object.assign(form, opportunity ? {
...
function isRequired(value: string | null) { return (value ?? "").trim().length > 0 || "Role title is required"; }
function nextStep() {
...
@media (max-width: 700px) { .opportunity-dialog { width: 100%; max-width: none; } .dialog-body { max-height: calc(100vh - 150px); padding: 16px; } .dialog-actions { position: sticky; bottom: -16px; margin: 8px -16px -16px; padding: 12px 16px; background: #fff; border-top: 1px solid #edf2f5; } }
</style>
